...
const Members = () => {
  ...
  const confirmChange = async (id, newDesignation) => {
    if (!token) return;
    ...
    try {
      const response = await axios.put(
        ...
        {
          userId: id,
          role: newDesignation.toLowerCase(), // API expects lowercase
        },
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (response.status === 200) {
        ...
        console.error("Role assignment failed with non-200 status");
      }
    } catch (error) {
    ...
    <div className="min-h-screen bg-white dark:bg-gray-900 text-black dark:text-white flex flex-col">
      ...
      <div className="max-w-6xl mx-auto py-6 flex-grow flex flex-col w-full  px-4">
                      ...
                      {isHr && (
                        <td className="px-4 py-2 border flex justify-center">
                          <select
                            value={member.role}
                            onChange={(e) =>
                              openConfirmationModal(member, e.target.value)
                            }
        ...
        <ConfirmDesignationModal
          ...
          onConfirm={() =>
            confirmChange(
              pendingChange.member._id,
              pendingChange.newDesignation
            )
          }
