...
const ViewInternOrUser = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        const res = await axios.get(
          ...
          }/intern/get-users-and-intern/${userId}`,
          {
            headers: {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-white dark:bg-gray-900 text-black dark:text-white flex flex-col flex-grow">
        <Navbar />
        <div className="max-w-4xl py-10 px-6 flex flex-col flex-grow text-black">
          Loading...
        </div>
        <Footer />
      </div>
  ...
  if (!userData) {
    return (
      <div className="min-h-screen bg-white dark:bg-gray-900 text-black dark:text-white flex flex-col flex-grow">
        <Navbar />
        <div className="max-w-4xl py-10 px-6 flex flex-col flex-grow text-alert">
          User not found
        </div>
        <Footer />
      </div>
    ...
    <div className="min-h-screen bg-white dark:bg-gray-900 text-black dark:text-white flex flex-col flex-grow">
      ...
      <div className="max-w-4xl py-10 px-6 flex flex-col flex-grow">
        ...
        <div className="bg-gray-100 dark:bg-gray-800 p-4 rounded shadow space-y-2">
          <p>
            <strong>ID:</strong> {renderValue(userData._id)}
          </p>
          <p>
            <strong>Name:</strong> {renderValue(userData.fullName)}
          </p>
          <p>
            <strong>Email:</strong> {renderValue(userData.email)}
          </p>
          <p>
            <strong>Role:</strong> {renderValue(userData.role)}
          </p>
        </div>
        ...
        {internData && (
          <div className="mt-10">
            ...
            <div className="bg-gray-100 dark:bg-gray-800 p-4 rounded shadow space-y-2">
              <p>
                <strong>College:</strong> {renderValue(internData.collegeName)}
              </p>
              <p>
                <strong>Branch:</strong> {renderValue(internData.branch)}
              </p>
              <p>
                <strong>DOB:</strong> {renderValue(internData.dob)}
              </p>
              <p>
                <strong>Aadhar:</strong> {renderValue(internData.aadhar)}
              </p>
              <p>
        ...
        {academics && (
          <div className="mt-10">
            ...
            <div className="bg-gray-100 dark:bg-gray-800 p-4 rounded shadow space-y-2">
              <p>
                <strong>Semester:</strong> {renderValue(academics.semester)}
              </p>
              <p>
                <strong>Session:</strong> {renderValue(academics.session)}
              </p>
            </div>
          </div>
        )}
