...
const MainButtons = ({ title, path, onClick, className = "" ,disabled=false}) => {
 ...
 const handleClick = async (e) => {
  ...
  return (
    <button
      onClick={handleClick}
      className={className==""?"cursor-pointer px-4 py-2 rounded-full text-sm bg-secondary text-white font-medium shadow-md hover:bg-primary dark:hover:bg-primary  transition-all" :className}
    disabled={disabled}
    >
      {title}
    </button>
